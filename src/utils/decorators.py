...
def log_with_context(**context: Any) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            adapter = logging.LoggerAdapter(logger, {'extras': context})

            # `adapter`를 사용하여 로그를 기록하거나, 함수에서 `logger` 대신 사용
            adapter.info(f"Entering function {func.__name__} with context: {context}")
            result = func(*args, **kwargs)
            adapter.info(f"Exiting function {func.__name__} with context: {context}")
