...
class LogStreamProcessor:
    ...
    def process_streaming_content(self, content_stream: Generator[Tuple[str, str], None, None]) -> Generator[Dict[str, Any], None, None]:
        ...
        buffer_size = 0
        current_metadata = LogParsingResult()  # 딕셔너리 대신 LogParsingResult 인스턴스 사용
                ...
                for line in chunk_content.split('\n'):
                    ...
                    if line.startswith('# Time:'):
                        ...
                        if match := self._patterns['query_stats'].search(line):
                            current_metadata.query_time = float(match.group(1))
                    ...
                    if buffer_size + line_size > self._max_buffer_size:
                        ...
                        current_block = []
                        buffer_size = 0
                        current_metadata = LogParsingResult()  # 새로운 인스턴스 생성
                        continue
        ...
        try:
            ...
            for line in content.split('\n'):
                ...
                if line.startswith('# Time:'):
                    # 이전 블록 처리
                    if current_block:
                        # yield from 결과를 리스트로 변환하여 처리 여부 확인
