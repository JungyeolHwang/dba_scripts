...
class ProcessingStats:
    ...
    Attributes:
        processed (int): 처리된 쿼리 수
        indexed (int): 인덱싱된 문서 수
        errors (int): 발생한 에러 수
        start_time (float): 처리 시작 시간
        batch_stats (List[Dict]): 배치 처리 통계
    """
    processed: int = 0
    indexed: int = 0
    errors: int = 0
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_batch_duration': sum(s['duration_seconds'] for s in self.batch_stats) / len(self.batch_stats) if self.batch_stats else 0
        }
...
class SlowQueryLogProcessor:
    ...
    def _process_batch(self) -> None:
        ...
        try:
            ...
            logger.info(f"배치 처리 완료 (인덱싱: {indexed}, 소요 시간: {duration:.2f}초)")
