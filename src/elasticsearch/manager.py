...
class ElasticsearchManager:
    ...
    def bulk_index(
        ...
        try:
            ...
            logger.info(f"전체 인덱싱 결과 - 성공: {total_success}, 실패: {total_failed}")
        ...
        while retry_count <= max_retries:
            try:
                ...
                bulk_results = helpers.bulk(
                    ...
                    stats_only=True  # 통계만 반환하도록 설정
                )
                
                # bulk_results는 (success_count, error_list) 튜플 반환
                success_count = bulk_results[0] if isinstance(bulk_results, tuple) else 0
                ...
                logger.info(f"배치 처리 완료 - 성공: {success_count}, 실패: {failed_count}")
